...
def vt_dialogEditTicketInfo():
    ...
    with st.form("frm_edit_ticket"):     
        ...
        with col1:
            ...
            st.markdown(f"Requested By: {currentTicket['REQUESTED_BY'] or '-'}")
        ...
        with col2:
            ...
            vt_newAppDate = st.date_input("Approving Date", value=vt_currentAppDate, key="vt_newAppDate")
            # ROW 3
            ...
            vt_newCallbackDate = st.date_input("Callback Date", value=vt_currentCallbackDate, key="vt_newCallbackDate")
        ...
        with col3:
            ...
            if st.form_submit_button("Save Changes"):
                ...
                newTicketDict["APPROVING_DATE"] = str(vt_newAppDate)
                newTicketDict["CALLBACK_DATE"] = str(vt_newCallbackDate)
                newTicketDict["CALLBACK_TIME"] = str(vt_newCallbackTime)
                newTicketDict["LAST_MODIFIED_BY"] = st.session_state.auth_user
                newTicketDict["LAST_MODIFIED_DTTM"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                # SAVE HEADER - JSONL
