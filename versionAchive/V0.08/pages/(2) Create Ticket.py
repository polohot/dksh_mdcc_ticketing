...
def ct_listAllJsonl(ct_folderPath):
...
def ct_getLastRecordOfJsonl(fullPath):
    try:
        last_line = None
...
def ct_dialogConfirm():
    ...
    for ttype in ['Material','Customer','Vendor']:
        lsFile = ct_listAllJsonl(f'ticketHeader/{ttype}/')
    ...
    if st.session_state.ct_showProgressBar == True:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            futures = [executor.submit(ct_getLastRecordOfJsonl, fullPath) for fullPath in lsFullPath[:100]]            
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            # Submit all file load tasks immediately
            futures = [executor.submit(ct_getLastRecordOfJsonl, fullPath) for fullPath in lsFullPath]            
            # Process them as they finish (as_completed) to update the bar fluidly
            total_files = len(futures)
            for i, future in enumerate(concurrent.futures.as_completed(futures)):
                result = future.result()
    ...
    if st.button("Go to Ticket", use_container_width=True):
        ...
        st.session_state["vt_selTicketHeaderPath"] = f"ticketHeader/{ticket_type}/{ticket_num}.jsonl"
